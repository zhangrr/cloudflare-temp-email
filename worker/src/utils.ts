...
/**
 * Parse JSON value from string or object
...
/**
 * Get JSON setting from database
 * @template T - Expected return type
 * @param c - Hono context
 * @param key - Setting key
 * @returns Promise with parsed setting or null
 */
 ...
 * @param c - Hono context
 * @param key - Setting key
 * @returns Promise with setting value or null
 */
...
export const checkCfTurnstile = async (
    ...
    if (!captchaRes.success) {
        console.error("Turnstile verification failed:", captchaRes);
...
export const checkUserPassword = (password: string) => {
    if (!password || password.length < 1 || password.length > 100) {
        throw new Error("Invalid password: must be between 1 and 100 characters")
    }
    return true;
}
