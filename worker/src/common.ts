...
/**
 * Validate email address name against configured regex
 * @param c - Hono context
 * @param name - Address name to validate
 * @throws Error if name doesn't match regex
 */
const checkNameRegex = (c: Context<HonoCustomType>, name: string) => {
    try {
        ...
        if (!regex.test(name)) {
            throw new Error(`Name "${name}" does not match required pattern: /${regexStr}/`);
        }
    }
    catch (e) {
        console.error("Failed to validate address regex:", e);
        throw e;
    }
}

/**
 * Get regex pattern for cleaning address names
 * @param c - Hono context
...
const getNameRegex = (c: Context<HonoCustomType>): RegExp => {
    try {
        ...
        console.error("Failed to get address regex, using default:", e);
    }
    return DEFAULT_NAME_REGEX;
}

/**
 * Update address last updated timestamp
 * @param c - Hono context
...
export async function updateAddressUpdatedAt(
    ...
    try {
        ...
        console.warn("Failed to update address timestamp:", e);
    }
}

/**
 * Generate random password for address
 * @returns Random password string (8 characters)
 */
...
/**
 * Check if address name is in block list
 * @param c - Hono context
 * @param name - Address name to check
 * @throws Error if name is blocked
 */
const checkNameBlockList = async (
    ...
    try {
        ...
        console.error("Failed to load address block list:", error);
    }
    if (blockList.some((item) => name.includes(item))) {
        throw new Error(`Address name "${name}" is blocked`);
    }
}

/**
 * Clean up old data based on type and age
 * @param c - Hono context
 * @param cleanType - Type of cleanup (inactiveAddress, mails, etc.)
 * @param cleanDays - Age threshold in days
...
export const cleanup = async (
    ...
    console.log(`Cleanup: ${cleanType} older than ${cleanDays} days`);
